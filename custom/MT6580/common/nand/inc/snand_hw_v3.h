...
*
*****************************************************************************/
#ifndef _SNAND_HW_V3_H_
#define _SNAND_HW_V3_H_

#include "SW_TYPES.H"
#include "hw_config.h"

//------------------------------------------------------------------------------
// REMARKS: Must be synchronized with ALPS definitions
//------------------------------------------------------------------------------
...
#define RW_SNAND_MAC_CTL              ((volatile uint32 *)(SNF_base + 0x0500))
#define RW_SNAND_MAC_OUTL             ((volatile uint32 *)(SNF_base + 0x0504))
#define RW_SNAND_MAC_INL              ((volatile uint32 *)(SNF_base + 0x0508))

#define RW_SNAND_RD_CTL1              ((volatile uint32 *)(SNF_base + 0x050C))
#define RW_SNAND_RD_CTL2              ((volatile uint32 *)(SNF_base + 0x0510))
#define RW_SNAND_RD_CTL3              ((volatile uint32 *)(SNF_base + 0x0514))

#define RW_SNAND_GF_CTL1              ((volatile uint32 *)(SNF_base + 0x0518))
#define RW_SNAND_GF_CTL3              ((volatile uint32 *)(SNF_base + 0x0520))

#define RW_SNAND_PG_CTL1              ((volatile uint32 *)(SNF_base + 0x0524))
#define RW_SNAND_PG_CTL2              ((volatile uint32 *)(SNF_base + 0x0528))
#define RW_SNAND_PG_CTL3              ((volatile uint32 *)(SNF_base + 0x052C))

#define RW_SNAND_ER_CTL               ((volatile uint32 *)(SNF_base + 0x0530))
#define RW_SNAND_ER_CTL2              ((volatile uint32 *)(SNF_base + 0x0534))

#define RW_SNAND_MISC_CTL             ((volatile uint32 *)(SNF_base + 0x0538))
#define RW_SNAND_MISC_CTL2            ((volatile uint32 *)(SNF_base + 0x053C))

#define RW_SNAND_DLY_CTL1             ((volatile uint32 *)(SNF_base + 0x0540))
#define RW_SNAND_DLY_CTL2             ((volatile uint32 *)(SNF_base + 0x0544))
#define RW_SNAND_DLY_CTL3             ((volatile uint32 *)(SNF_base + 0x0548))
#define RW_SNAND_DLY_CTL4             ((volatile uint32 *)(SNF_base + 0x054C))

#define RW_SNAND_STA_CTL1             ((volatile uint32 *)(SNF_base + 0x0550))
#define RW_SNAND_CNFG                 ((volatile uint32 *)(SNF_base + 0x055C))

#define RW_SNAND_GPRAM_DATA           ((volatile uint32 *)(SNF_base + 0x0800))
...
/*------------
 * Some vendor's SPI-NAND has higher than 8-bit device ECC capability, therefore we must provide the same or higher ECC capability if we use MTK ECC.
 * However, some SPI-NAND can't access spare area more than 64 bytes in normal way (by NFI HW), thus we split ECC capability to
 * 1. Preloader(includeing bootloader header)   : Use ECC 4-bit. BootROM has only ECC 4 bit capability if BootROM can only access 64 bytes spare area.
 * 2. Others                                    : Use ECC 12-bit
 *
 * REMARKS! Using this kind of SPI-NAND, we must set spare_size as 64 thus preloader image with ECC 4-bit information can be created and downloaded.
 *------------*/
#define SNAND_ADV_ECC_SPLIT                 (0x00000001)

/*------------
 * SPI-NAND Spare Format
 *------------*/
#define SNAND_SPARE_FORMAT_1                (0x80000000)    // 64 Bytes: 8 Byte Reserved | 8 Byte Avaliable | 8 Byte Reserved | 8 Byte Avaliable | 8 Byte Reserved | 8 Byte Avaliable | 8 Byte Reserved | 8 Byte Avaliable

// ID related definition
#define SNAND_MAX_ID                        (5)
