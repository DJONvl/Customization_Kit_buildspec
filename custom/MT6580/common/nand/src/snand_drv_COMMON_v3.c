...
#if defined(BB_MT6572)  // should be 72, use 71 for dev (fixme)
...
#elif defined(BB_MT6580)    // should be 71, use 72 for dev (fixme)
...
STATUS_E  SNAND_COMMON_PageRead_V3(
    ...
    if (row_addr > NUTL_TOTAL_PAGE_COUNT())
    {
        DBG_MSG("[SNAND_COMMON_PageRead_V3] S_DA_INVALID_RANGE! row %d\n\r", row_addr);
        return S_DA_INVALID_RANGE;
    }
    ...
    */
    ...
    #if defined(_SNAND_DEBUG)
    DBG_MSG("R  (r: %d, b: %d, ecc: %d), sec:%d, page_size:%d\n", row_addr, row_addr / 64, g_snand_ecc_bit_now, sectors, NUTL_PAGE_SIZE());
    #endif
...
STATUS_E  SNAND_COMMON_PageProgram_V3(
    ...
    if (row_addr > NUTL_TOTAL_PAGE_COUNT())
    {
        DBG_MSG("[SNAND_COMMON_PageProgram_V3] S_DA_INVALID_RANGE! row %d\n\r", row_addr);
        return S_DA_INVALID_RANGE;
    }
                                         ...
                                         const u32 *p_page_spare)
{
    ...
    if (row_addr > NUTL_TOTAL_PAGE_COUNT())
    {
        DBG_MSG("[SNAND_COMMON_PageSpareProgram_V3] S_DA_INVALID_RANGE! row %d\n\r", row_addr);
        return S_DA_INVALID_RANGE;
    }
...
STATUS_E SNAND_COMMON_Randomizer_Config(bool use ,uint32 pageNo)
{
	return S_DONE;
}
...
const NAND_CMD_Callback_S	g_SNAND_COMMON_CB_FUNC_SET={
	...
	,SNAND_COMMON_Randomizer_Config
};
