...
const NAND_CMD_Callback_S	g_NAND_COMMON_CB_FUNC_SET={
	...
	,NAND_COMMON_Randomizer_Drv_Config
       ...
       ,NAND_COMMON_PageProgramBad_V5
       ,NAND_COMMON_Block_charge_V5
};
...
void HYNIX_FDIE_Set_RR_Para(uint32 rr_index, uint32 def)
{
    ...
    for(reg_cunt=0; reg_cunt<4; reg_cunt++){
	 ...
	 //reg_val = hynix_rr_table_fdie.rr_table[real_hynix_rr_table_idx_fdie].rr_table[rr_index*4+reg_cunt];
        ...
        //MSG(DEBUG, "W[%x]0x%x ",(0x38+reg_cunt),reg_val);
    ...
    if(def){
        ...
        //NFI_Wait_Return_Ready(timeout);
        //if(!timeout){
        //    MSG(DEBUG, "HYNIX_set_RR_Para for RB timeout\n");
        //}  
    }
 #endif   
    //MSG(DEBUG, "\n");
...
static void mtk_nand_parapage_15nm_rrtry(U32 retryCount, U32 def)
{
	...
	//MSG(DEBUG, "mtk_nand_parapage_15nm_rrtry  at count:%d def:%d\n", retryCount, def);
...
STATUS_E NAND_COMMON_Switch_Sync(bool sync)
{
	uint32 reg1, reg2, reg3,delayctrl,reg4;
	reg1 = DRV_Reg32(NFI_DEBUG_CON3);
	reg2 = DRV_Reg32(ECC_BYPASS);
	reg3 = DRV_Reg32(NFI_1X_INFRA);
	//reg4 = DRV_Reg(NFI_NAND_TYPE_CNFG_REG32);
	delayctrl = NUTL_DLY_MUX()<<24|NUTL_DLY_CTRL();
	if(sync) // toggle, onfi
	{
		reg1 &= ~0x8000;
		reg2 &= ~0x1;
		reg3 = 0xA950D2;
		reg4 = 1;
		DRV_WriteReg32(NFI_DEBUG_CON3,reg1);
		...
		DRV_WriteReg32(NFI_DQS_DELAY_CTRL,delayctrl);
		DRV_WriteReg32(NFI_DELAY_CTRL,0xA001);
		while(0 == (DRV_Reg(NFI_STA) && STA_FLASH_MACRO_IDLE))
up:
		DRV_WriteReg32(NFI_NAND_TYPE_CNFG_REG32 , reg4);
		MSG(DEBUG, "NFI_NAND_TYPE_CNFG_REG32 0x%x reg 0x%x xxx\n",DRV_Reg(NFI_NAND_TYPE_CNFG_REG32),reg4);
		if(DRV_Reg(NFI_NAND_TYPE_CNFG_REG32) == 4)
			goto up;
	}
	else
	{
...
up1:
		DRV_WriteReg32(NFI_NAND_TYPE_CNFG_REG32,reg4);
		MSG(DEBUG, "NFI_NAND_TYPE_CNFG_REG32 0x%x reg 0x%x xxx\n",DRV_Reg(NFI_NAND_TYPE_CNFG_REG32),reg4);
		if(DRV_Reg(NFI_NAND_TYPE_CNFG_REG32) == 1)
			goto up1;
...
STATUS_E NAND_COMMON_Randomizer_Config(bool use, uint32 pageNo)
{
	...
	if(device_need_rand)
	{
		...
		if(use)
		{
			...
			//MSG(DEBUG, "pageNo 0x%x seed 0x%x\n",pageNo,seed);
			nfi_ran_cnfg = 0;
...
STATUS_E NAND_COMMON_Interface_Config(bool use)
{
	uint32 array[4] = {0,0,0,0};
	if(use)
	{
		if(NUTL_IO_INTERFACE_SYNC() == NAND_IO_TOGGLE ||NUTL_IO_INTERFACE_SYNC() == NAND_IO_ONFI)
		{
			if(ddr_interface == FALSE)
			{
				...
				MSG(DEBUG, "interface change 0x%x\n",array[0]);
			}
			ddr_interface = TRUE;
		}
	}
...
#define MSDC0_SELGP_SET             (0x10015000 + 0x00AC)
#define NFI_PULLEN_MODE             (0x10015000 + 0x009C)
...
#if defined(MT6572) || defined(BB_MT6580) 
...
// MSG(DEBUG, "MSDC0_SELGP_SET 0x%x NFI_PULLEN_MODE 0x%x \n",DRV_Reg32(MSDC0_SELGP_SET),DRV_Reg32(NFI_PULLEN_MODE));
    ...
    MSG(DEBUG, "reset takes %d, status 0x%x\n",start_tick,*NFI_STA); //for record
#endif    
...
//	if(*id1 == 0xA794DEAD ||*id1 ==  0xA714DEAD||*id1 ==  0x9E14D7AD)
//		device_need_rand = TRUE;
	//toshiba 15nm
//	if(*id1 == 0x9394DE98 ||*id1 ==  0x93943A98)
	device_need_rand = TRUE;
...
STATUS_E  NAND_COMMON_BlockErase_V5(
...
#endif
	//MSG(DEBUG, "Drv NAND_COMMON_BlockErase_V5.....%x\n",row_addr);
#if defined(__NFI_SUPPORT_TLC__)
	...
	*NFI_CMD = g_pNandInfo->m_dev->m_cmd_set->m_erase_cnf.m_cmd;
	...
	{
		MSG(DEBUG, "[xiaolei] eb fail 0x%x\n",row_addr);
		return S_DA_NAND_ERASE_FAILED;
	}
	if(status != S_DONE)
	{
		MSG(DEBUG, "[xiaolei] eb fail 2  0x%x\n",row_addr);
		return status;
	}
...
//------------------------------------------------------------------------------
STATUS_E  NAND_COMMON_Block_charge_V5(const uint32 row_addr)
{
	if(TNAND_TC58TEG6TGLTA00 ==  NUTL_DEVICE_NAME() || TNAND_TC58TEG7THL ==  NUTL_DEVICE_NAME()) {
	uint32	page_size;
	uint32	addr_cycle;
	uint32	row_addr_nob, col_addr_nob;
	uint32  c_timeout = NFI_PSTA_TIMEOUT;
...
STATUS_E  NAND_COMMON_PageProgramBad_V5(
				const uint32  c_timeout
				,const uint32  row_addr
                                                        ...
                                                        uint32  ecc_parity_from_reg[8])
{
    ...
    MSG(DEBUG, "page 0x%x,sector_buff=0x%x, tlc_program_cycle=%d \n\r", page_index, sector_buff, tlc_program_cycle);
...
static unsigned char local_spare_buf[2048];
static unsigned char local_page_buf[16384];
...
//xiaolei macro reset
...
#endif
		//MSG(DEBUG, "row_addr 0x%x\n",row_addr);
		
		NAND_COMMON_Interface_Config(TRUE);
               NFI_Reset();
		//MSG(DEBUG, "NFI_NAND_TYPE_CNFG_REG32 0x%x\n",*NFI_NAND_TYPE_CNFG_REG32);
 ...
 while(logical_plane_index < logical_plane_num)
		...
		*NFI_CON = (CON_NFI_BRD | (sectors << CON_NFI_SEC_SHIFT));
		...
		// 7. Read data
		// read page data 
		if( S_DONE != (ret=NUTL_FIFO_Read_V5(c_timeout, bUsingDMA, p_data32+colu_addr/4, page_size/logical_plane_num)) ) {
			//goto end;
		}
		if( disautofmt == FALSE)
	...
	{
		//MSG(DEBUG, "[pg] %d empty\n", page_addr);
		if(bad_block_remark_done)
		{
			//MSG(DEBUG, "[pg] reset 0xFF\n");
    ...
    if((readRetryCount != 0) && NUTL_DEVICE_MLC())
	{
		...
		else if(NUTL_READ_RETRY_TYPE() == RTYPE_HYNIX_16NM)
		{
			...
			//MSG(DEBUG, "readtry OK at page:%d count:%d\n",page_addr, readRetryCount);
		}
		else if(NUTL_READ_RETRY_TYPE() == RTYPE_HYNIX_FDIE)
		{
...
if(NUTL_NAND_TYPE() != NAND_FLASH_TLC){
	...
	*NFI_CON = (CON_NFI_BWR | (sectors << CON_NFI_SEC_SHIFT));
	...
	if( S_DONE != (ret=NUTL_FIFO_Write_V5(bUsingDMA, p_data32, page_size,spare_size, NUTL_PROGRAM_PAGE_ONLY)) ) {
		return ret;
	}
...
if(use_randomizer)
    ...
    *NFI_CON = (CON_NFI_BWR | (sectors << CON_NFI_SEC_SHIFT));
	...
	// Program data
    if (S_DONE != (ret=NUTL_FIFO_Write_V5(FALSE, p_page_spare, page_size, spare_size, NUTL_PROGRAM_PAGE_SPARE)))
    {
        return ret;
    }
