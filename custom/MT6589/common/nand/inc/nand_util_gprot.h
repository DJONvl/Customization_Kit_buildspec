...
typedef STATUS_E (*F_NAND_UTIL_CHECK_RANGE_T)(const uint32 absolute_addr, const uint32 length, const NUTL_AddrTypeFlag_E addr_type);
...
typedef STATUS_E (*F_NAND_UTIL_READ_ID)(const uint32 c_timeout, uint16 * p_maker_code, uint16 * p_device_code, uint16 * p_ext_code1, uint16 * p_ext_code2, uint16 * p_ext_code3);
