...
;2012-3-29 Ning.cui
;@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
INT_SRAM_BASE	EQU		0x100000
INT_SRAM_OFFSET	EQU		0xE000
STACK_PTR		EQU		0x2000
    IMPORT	C_Main
    IMPORT	INT_InitRegions
    IMPORT  ||Image$$INT_SYSRAM$$Base||
    IMPORT  ||Image$$FREE_INT_SYSRAM_BEGIN$$Base||   
    IMPORT      arm_cp15_dcache_ctrl      
    IMPORT      arm_cp15_dcache_clean_invalidate
    IMPORT      arm_cp15_icache_invalidate
    IMPORT      arm_cp15_DSB
    IMPORT      arm_cp15_ISB
    IMPORT      arm_cp15_mmu_ctrl
    IMPORT      arm_cp15_unalignment_access_ctrl
    IMPORT      arm_cp15_alignment_fault_ctrl
    IMPORT      arm_cp15_arm_v6_pagetable_ctrl
    IMPORT      arm_cp15_branch_prediction_ctrl
    IMPORT      arm_cp15_mmu_ttbr0_set
    IMPORT      arm_cp15_disable_smp
    IMPORT      arm_cp15_enable_smp
    IMPORT      arm_cp15_icache_ctrl
    ENTRY
Start	
    B     ResetHandler
...
ResetHandler
    ...
    SUB		r1, r1, #0x10
    MOV		sp, r1
    STMFD   sp!, {r4-r5} ; Backup DA argument    

    ; invalidate I-cache / D-cache
    MOV     r0, #0
    BLX     arm_cp15_dcache_ctrl
    BLX     arm_cp15_dcache_clean_invalidate
    BLX     arm_cp15_DSB
    BLX     arm_cp15_icache_invalidate
    BLX     arm_cp15_ISB
    MOV     r0, #0
    BLX     arm_cp15_branch_prediction_ctrl
    MOV     r0, #0
    BLX     arm_cp15_mmu_ctrl
    MOV     r0, #0
    BLX     arm_cp15_unalignment_access_ctrl
    MOV     r0, #0
    BLX     arm_cp15_alignment_fault_ctrl
    MOV     r0, #0
    BLX     arm_cp15_arm_v6_pagetable_ctrl
    MOV     r0, #0    
    BLX     arm_cp15_mmu_ttbr0_set
    BLX     arm_cp15_disable_smp
    
    LDMIA   sp!, {r4-r5}   ; Restore DA argument
    ENDIF 
    ; Disable interrupt 
    MRS   r0, cpsr 
    MOV   r1, #(I_BIT|F_BIT)
...
InitCache
    ; enable Icache + branch prediction */
