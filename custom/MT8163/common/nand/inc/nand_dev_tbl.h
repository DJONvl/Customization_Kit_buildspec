...
typedef STATUS_E  (*FP_CB_NAND_PAGE_READ_T)(const uint32  c_timeout, const uint32 row_addr, uint32 *p_data32, uint32  ecc_parity_from_reg[8]);
typedef STATUS_E  (*FP_CB_NAND_PAGE_PROGRAM_T)(const uint32  c_timeout, const uint32 row_addr, const uint32 *p_data32, uint32  ecc_parity_from_reg[8]);
#else
...
typedef STATUS_E  (*FP_CB_NAND_RANDOMIZER_CONFIG)(bool use);
...
//#if defined(__NFI_SUPPORT_TLC__)
	uint64		m_total_size;//if TLC mode unit is bytes, others is MB
//#else
	// total size in MB 
//	uint64		m_total_size_in_mb;
//#endif
	// pages per block 
...
typedef struct
{
	...
	bool		two_plane; //support two physical plane operation.
} NFI_TLC_FEATURE;
