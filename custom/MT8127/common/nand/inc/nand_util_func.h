...
#define PL_PAGE_NUMBER  (NUTL_PAGES_PER_BLOCK() * 4) //4blocks
#endif

//#define NAND_SYNCMODE_TEST //xiaolei  for nand sync mode test
...
#define NUTL_PPAGE_TRANSLATE(x)							(g_pNandInfo->m_dev->m_mlc_setting->m_ppage_tbl.table[x].low_page)
#define NUTL_PPAGE_HIGH_PAGE(x)							(g_pNandInfo->m_dev->m_mlc_setting->m_ppage_tbl.table[x].high_page)
#define NUTL_DEVICE_MLC()                                                   ((g_pNandInfo->m_dev->m_mlc_setting) && (g_pNandInfo->m_dev->m_nand_type == NAND_FLASH_MLC))
...
#define NUTL_READ_RETRY_PRECMD()					         (g_pNandInfo->m_dev->m_mlc_setting != NULL?g_pNandInfo->m_dev->m_mlc_setting->read_retry_precmd: 0xFF)
#define NUTL_READ_RETRY_VALUE(i)					         (g_pNandInfo->m_dev->m_mlc_setting != NULL?g_pNandInfo->m_dev->m_mlc_setting->read_retry_tbl[i]: 0xFF)
#define NUTL_READ_RETRY_TYPE()					                (g_pNandInfo->m_dev->m_mlc_setting != NULL?g_pNandInfo->m_dev->m_mlc_setting->read_retry_type: 0xFF)
#define NUTL_CHIP_NUM()								     (g_pNandInfo->m_dev->m_hw_info.m_chip_num)
...
#define NUTL_2P_TLC()								(g_pNandInfo->m_dev->m_hw_info.two_plane)
...
#define	CB_NAND_RANDOMIZER_CONFIG(use)	                            FP_CB_NAND_RANDOMIZER_CONFIG()(use)
