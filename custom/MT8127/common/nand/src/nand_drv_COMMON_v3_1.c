...
void NFI_RANDOMIZER_ENCODE(U32 u4PgIdx, bool fgRandom, bool fgSS, bool fgPage, U32 u4SectorIdx)
{
	...
	if(fgRandom)
	{
		if(fgSS) //samsung randomizer
		{
			...
			#endif
		//DBG_MSG("en pg %d rand 0x%x\n",u4PgIdx,SS_RANDOM_SEED[u4PgIdx & (SS_SEED_NUM-1)]);
...
//------------------------------------------------------------------------------
STATUS_E  NAND_COMMON_Block_charge_V3(const uint32 row_addr)
{
	...
	uint32     page_addr = (uint32)row_addr;
	NFI_TLC_WL_INFO  tlc_wl_info;
	//DBG_MSG("Drv NAND_COMMON_BlockErase_V3.....%x\n",row_addr);
       U32 reg_val = 0;
       U32 u4EraseTimes = 1;
	page_size = NUTL_PAGE_SIZE();
	addr_cycle = NUTL_ADDR_CYCLE();
		...
		if(g_pNandInfo->m_dev->m_tlc_setting->slcopmodeEn) // slc mode
	           {
	               if(0xFF != g_pNandInfo->m_dev->m_tlc_setting->en_slc_mode_cmd)
	               {
				     //DBG_MSG("<Bayi>0xA2 cmd, 0x%x, 0x%x\n", g_pNandInfo->m_dev->m_tlc_setting->en_slc_mode_cmd, g_pNandInfo->m_dev->m_tlc_setting->dis_slc_mode_cmd);
				     reg_val = DRV_Reg(NFI_CNFG);
		                   reg_val &= ~CNFG_READ_EN;
		                   reg_val &= ~CNFG_OP_MODE_MASK;
		                   reg_val |= CNFG_OP_CUST;
                                                        ...
                                                        uint32  ecc_parity_from_reg[8])
{
	...
	if(ftl_img && NUTL_2P_TLC() && (!bl_region_ing))
	{
             ///DBG_MSG("write 2p , (page_index+page_per_block):0x%x, nfi_sector_num:0x%x \n", (page_index+page_per_block), nfi_sector_num);
                                ...
                                NFI_TLC_PG_CYCLE program_cycle)
{
    ...
    if( S_DONE != status )
    {
        ...
        main_ptr += chunk_size;// g_pNandInfo->m_dev->m_hw_info.m_page_size;
...
STATUS_E  NAND_COMMON_TLC_BlockProgram(const uint32  c_timeout, U32 blk_index, P_U32 sector_buff, P_U32 spare_buff)
{
    ...
    P_U8 main_ptr = NULL;
    STATUS_E  status    = S_DONE;
    DBG_MSG("line=%d,function= %s ,blk_index=%d \n ",  __LINE__,  __FUNCTION__, blk_index);
	...
	
	{
	 ...
	 for(idx = 0; idx < pg_time_per_block; idx++)
	 {
		 DBG_MSG("Program page 0x%x\n\r", (base_idx + idx));
			 ...
			 main_ptr += g_pNandInfo->m_dev->m_hw_info.m_page_size;
		 status = NAND_COMMON_PageProgram_V3(NFI_PSTA_TIMEOUT, (base_idx+idx), (P_U32)main_ptr, spare_buff, ecc_parity_from_reg);
		 if( S_DONE != status )
			{
				return status;
			}
...
void dump_nfi(void) {
	...
	DBG_MSG( "ECC_DECDONE: 0x%x\n", *ECC_DECDONE);
	DBG_MSG( "ECC_DECFER: 0x%x\n", *ECC_DECFER);
	DBG_MSG( "ECC_DECFSM: 0x%x\n", *ECC_DECFSM);
	DBG_MSG( "NFI_INTR: 0x%x\n", *NFI_INTR);
	DBG_MSG( "NFI_INTR_EN: 0x%x\n", *NFI_INTR_EN);
...
STATUS_E  NAND_COMMON_PageRead_V3_hw(
				const uint32  c_timeout
				,const uint32  row_addr
				,uint32 *p_data32 /* MUST be 32bits alignment addr */
				,uint32 *p_spare32 /* MUST be 32bits alignment addr */
...
U32 reg_val = 0;
	...
	NFI_TLC_WL_INFO  tlc_wl_info;
       ...
       U32   nfi_sector_num = g_pNandInfo->m_dev->m_hw_info.m_page_size /g_pNandInfo->m_dev->m_hw_info.m_sector_size;
#endif
	//DBG_MSG("[RP] 0x%x  0x%x 0x%x\n", row_addr, (U32)p_data32, (U32)p_spare32);
	if(reset_fdm_and_ecc) {
	    if(page_addr < second_part_start_page) {
               mtk_nand_reset_fdm_ecc(FALSE);
	    } else {
...
//				   DBG_MSG("%d %s 0x%x\n",__LINE__,__FUNCTION__,g_pNandInfo->m_dev->m_tlc_setting->en_slc_mode_cmd);
	...
	if(g_pNandInfo->m_dev->m_nand_type  == NAND_FLASH_AMLC) {
		if(g_pNandInfo->m_dev->m_tlc_setting->slcopmodeEn) // slc mode
				...
				if(NUTL_MAKER_CODE() == 0x45) // sandisk amlc nand
   				 	...
   				 	//DBG_MSG("%d %s row_addr:0x%x page_addr:0x%x\n",__LINE__,__FUNCTION__, row_addr, page_addr);
 				}
		//				   DBG_MSG("%d %s 0x%x\n",__LINE__,__FUNCTION__,g_pNandInfo->m_dev->m_tlc_setting->en_slc_mode_cmd);
	               }
	           }
	...
	do{
	page_size = NUTL_PAGE_SIZE();
	if((g_pNandInfo->m_dev->m_nand_type == NAND_FLASH_TLC) && NUTL_2P_TLC() && (!bl_region_ing))
		page_size >>= 1;
	spare_size = NUTL_SPARE_SIZE();
	column_addr_bits = NUTL_PAGE_ADDR_SHIFT_BITS();
	addr_cycle = NUTL_ADDR_CYCLE();
...
) 
{
	...
	if((g_pNandInfo->m_dev->m_nand_type == NAND_FLASH_TLC) && NUTL_2P_TLC() && (!bl_region_ing))
			nfi_sector_num >>= 1;
		tlc_snd_phyplane = FALSE;
	NAND_COMMON_PageRead_V3_hw(c_timeout, row_addr, (uint32*)main_ptr, (uint32*)spare_ptr, ecc_parity_from_reg);
	...
	if((g_pNandInfo->m_dev->m_nand_type == NAND_FLASH_TLC) && NUTL_2P_TLC() && (!bl_region_ing))
	{
		...
		main_ptr += (sector_size * nfi_sector_num);
    		if(p_spare32)
        		spare_ptr += (nfi_sector_num) * 8/*nfi_fdm_size*/;
		NAND_COMMON_PageRead_V3_hw(c_timeout, row_addr+page_per_block,(uint32*)main_ptr, (uint32*)spare_ptr, ecc_parity_from_reg);
		tlc_snd_phyplane = FALSE;
	}
...
//	bUsingDMA = TRUE;
	...
	*NFI_CNFG |= ((bUsingDMA)?(CNFG_AHB):0);									// AHB mode or NFI mode
	...
	// 3.1 program cmd 
	if ((g_pNandInfo->m_dev->m_nand_type  == NAND_FLASH_AMLC) 
		&& NUTL_2P_TLC() &&(tlc_snd_phyplane) && (!bl_region_ing))
		{
		//DBG_MSG("0x81 cmd\n");
