...
#define MMC_ERR_RPMB_KEY_EXISTED 13
...
dsr:1;
   ...
   struct mmc_ext_csd {
      ...
      unsigned int    sectors;	    /* 4G+ user data area size, will re-calculate while host execute partitions */
      unsigned int    hs_max_dtr;
      unsigned int    boot_part_sz;
      unsigned int    rpmb_sz;
      unsigned int    access_sz;
      unsigned int    part_gp1_sz;
      unsigned int    part_gp2_sz;
      unsigned int    part_gp3_sz;
      unsigned int    part_gp4_sz;
      unsigned int    enh_user_sz;
      unsigned int    enh_max_sz;
...
enh_usr:1;
      ...
      unsigned int erase_group_def;  /* "0" is to use old, "1" is to use high capcity */
   };

   typedef enum {
      EMMC_PART_ATTR_UNKNOWN = 0,
      EMMC_PART_ATTR_USR = 1,
      EMMC_PART_ATTR_GP1 = 2,
      EMMC_PART_ATTR_GP2 = 4,
      EMMC_PART_ATTR_GP3 = 8,
      EMMC_PART_ATTR_GP4 = 16,
      EMMC_PART_ATTR_END
   } EMMC_Part_Attr_E;
...
#define MMC_CAP_DDR		(1 << 7) /* The host support dual data rate */
   ...
   struct mmc_host
   {
      ...
      u32 base_top;   /* msdc_top base address */
...
#define MMC_TYPE_SDIO	    (0x00000004) /* SDIO card */
   ...
   struct mmc_card {
      ...
      u8			     addr_tran_enabled;  /* the address translation is enabled or disabled*/
   };
...
#define mmc_card_id(c)		    ((c)->host->id)
   ...
   extern u32 mmc_get_wpg_size(struct mmc_card *card);
   ...
   extern int mmc_set_ext_csd(struct mmc_card *card, uint8 addr, uint8 value);
   extern int mmc_send_status(struct mmc_host *host, struct mmc_card *card, u32 *status);
   extern int mmc_set_user_wp(struct mmc_card *card, u8 wp);
   extern int mmc_set_write_prot(struct mmc_card *card, u32 addr);
   extern int mmc_send_write_prot(struct mmc_card *card, u32 wp_addr, u32 *wp_status);
   extern int mmc_send_write_prot_type(struct mmc_card *card, u32 wp_addr, u32 *wp_type);
   extern int mmc_erase_is_support(int dev_num);
   extern int mmc_erase_is_support(int dev_num);
   extern int mmc_block_erase(int dev_num, unsigned long blknr, u32 blkcnt);
   extern int mmc_set_part_config(struct mmc_card *card, u8 cfg);
   extern int mmc_get_sandisk_fwid(int id, u8* buf);
   extern int mmc_get_cid(int id, u8* buf);
