...
#first da address: 256KB(D1), 128K(D2) (shared sram)
ADDRS.DA_INIT_BASE_ADDRESS 	   	:= 0x200000
ADDRS.DA_INIT_SIZE		 	   	:= 0x20000

#first da heap address: 64KB (sram)
ADDRS.DA_INIT_HEAP_BASE_ADDRESS  	:= 0x100000
ADDRS.DA_INIT_HEAP_BASE_SIZE	   	:= 0x10000
...
#add_da_info
BBCHIP_NAME					:=MT6739
BBCHIP_HW_CODE				:=0x6739
BBCHIP_HW_SUB_CODE			:=0x8A00
...
CFLAGS += -mcpu=cortex-a7 -mfloat-abi=hard -mfpu=neon
...
FEATURES.DA_MODE  := DA_MODE_BRINGUP

ifeq ($(FEATURES.DA_MODE), DA_MODE_FPGA)
FEATURES.CFG_FPGA_PLATFORM := 1
endif

#FEATURES.ENABLE_MSDC_DMA 	:= 1
FEATURE_SWITCHES_INIT.ENABLE_MSDC_DMA	:=0
FEATURE_SWITCHES_LOOP.ENABLE_MSDC_DMA  :=1
FEATURE_SWITCHES_INIT.ENABLE_USB_DMA	:=0
FEATURE_SWITCHES_LOOP.ENABLE_USB_DMA  :=1

FEATURES.DA_ENABLE_SECURITY := 0
