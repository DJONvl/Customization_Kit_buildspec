...
#define CFG_2CS_NAND (1) // bean add for 2cs
#define CFG_COMBO_NAND (1) // bean add for combo nand
...
typedef status_t  (*FP_CB_NAND_READ_ID_T)(const uint32  c_timeout, uint32* id1, uint32* id2);
typedef status_t  (*FP_CB_NAND_RESET_T)(const uint32  c_timeout);
typedef status_t  (*FP_CB_NAND_PRE_PROCESS_T)(const uint32  c_timeout);
typedef status_t  (*FP_CB_NAND_READ_STATUS_T)(const uint32  c_timeout);
typedef status_t  (*FP_CB_NAND_BLOCK_ERASE_T)(const uint32  row_addr);
typedef status_t  (*FP_CB_NAND_BAD_BLOCK_SYMBOL_CHECK_T)(const uint32  *p_spare32);
typedef status_t  (*FP_CB_NAND_BAD_BLOCK_SYMBOL_SET_T)(uint32  *p_spare32);
typedef status_t  (*FP_CB_NAND_PAGE_READ_T)(const uint32  c_timeout, const uint32 row_addr, uint32 *p_data32, uint32* p_spare32, uint32  ecc_parity_from_reg[8]);
typedef status_t  (*FP_CB_NAND_PAGE_PROGRAM_T)(const uint32  c_timeout, const uint32 row_addr, const uint32 *p_data32, const uint32 *p_spare32, uint32  ecc_parity_from_reg[8]);
typedef status_t  (*FP_CB_NAND_SPARE_READ_T)(const uint32  c_timeout, const uint32 row_addr, uint32 *p_spare32);
typedef status_t  (*FP_CB_NAND_SPARE_PROGRAM_T)(const uint32  c_timeout, const uint32 row_addr, const uint32 *p_spare32);
typedef status_t  (*FP_CB_NAND_PAGE_SPARE_PROGRAM_T)(uint32 row_addr, const uint32 *p_page_spare);
typedef status_t  (*FP_CB_NAND_RANDOMIZER_CONFIG)(bool use,uint32 page);
typedef status_t  (*FP_CB_NAND_BLOCK_PROGRAM_T)(const uint32  c_timeout, const uint32 blk_addr, const uint32 *p_data32, const uint32 *p_spare32);
typedef status_t  (*FP_CB_BAD_NAND_PAGE_PROGRAM_T)(const uint32  c_timeout, const uint32 row_addr, const uint32 *p_data32, const uint32 *p_spare32, uint32  ecc_parity_from_reg[8]);
typedef status_t  (*FP_CB_NAND_BLOCK_CHARGE)(const uint32  row_addr);
...
typedef status_t  (*FP_CB_NAND_OTP_ENTER_T)(const uint32  c_timeout);
typedef status_t  (*FP_CB_NAND_OTP_PAGE_READ_T)(const uint32  c_timeout, const uint32 row_addr, uint32 *p_data32, uint32  ecc_parity_from_reg[8]);
typedef status_t  (*FP_CB_NAND_OTP_PAGE_PROGRAM_T)(const uint32  c_timeout, const uint32 row_addr, const uint32 *p_data32, uint32  ecc_parity_from_reg[8]);
typedef status_t  (*FP_CB_NAND_OTP_LOCK_T)(const uint32  c_timeout);
typedef status_t  (*FP_CB_NAND_OTP_LOCK_CHECKSTATUS_T)(const uint32  c_timeout);
typedef status_t  (*FP_CB_NAND_OTP_EXIT_T)(const uint32  c_timeout);
...
typedef struct {
	...
	uint8		two_plane; //support two physical plane operation.
} NAND_HW_Info_S;
...
typedef struct{
	...
	const uint32                         read_retry_precmd;
	const uint32                         interface_address;
	const uint32                         interface_async;
	const uint32                         interface_sync;
	const NAND_RRTYPE             read_retry_type;
	const NAND_PPAGE_TBL        m_ppage_tbl;
	const uint32                         read_retry_tbl[32]; //read retry table
}NAND_MLC_FEATURE;

typedef struct
{
	uint8         slcopmodeEn;               //bool,TRUE: slc mode   FALSE: tlc mode
	uint8		 normalslcopen;
	uint8         pPlaneEn;                     //bool, this chip has pseudo plane
	uint8         needchangecolumn;    //bool, read page with change column address command
	uint8         normaltlc;        //bool,whether need 09/0d 01/02/03 tlc command, whether need wl and page exchange.  micron no need
	...
	uint32		normal_slc_mode_cmd;
       uint32           ecc_required;
	uint8         ecc_recalculate_en;  //for nfi config;bool
...
struct _NAND_DeviceInfo_S {
	...
	//attention: if modify this struct, please rebuild securitylib.
...
typedef struct {
	uint8	m_data[32768];
	uint8	m_spare[128*32];
} NAND_Page32768_8_S;
typedef struct {
	uint16	m_data[16384];
	uint16	m_spare[128*16];
} NAND_Page32768_16_S;

typedef struct {
	uint32	m_data[8192];
	uint32	m_spare[128*8];
} NAND_Page32768_32_S;
...
typedef union {
	uint8				m_raw8[32768+128*32];
	uint16				m_raw16[16384+64*32];
	uint32				m_raw32[8192+32*32];

	NAND_Page32768_8_S	m_pagespare8;
	NAND_Page32768_16_S	m_pagespare16;
	NAND_Page32768_32_S	m_pagespare32;

       NAND_Page1024_8_S	m_sectorspare8[32];
	NAND_Page1024_16_S	m_sectorspare16[32];
	NAND_Page1024_32_S	m_sectorspare32[32];
} NAND_Page32768_U;
