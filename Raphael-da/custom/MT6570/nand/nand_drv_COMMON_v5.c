...
up:
	...
	else
	{
		...
		DRV_WriteReg(NFI_DEBUG_CON3,reg1);
				...
				)
{
	...
	LOGV("[%s] LINE %d NFI_CNFG 0x%x\n", __func__, __LINE__,*NFI_CNFG);
	...
	LOGV("[%s] LINE %d NFI_STA 0x%x\n", __func__, __LINE__,*NFI_STA);
...
//xiaolei macro reset
		...
		NFI_Reset();
		//DBG_MSG("NFI_NAND_TYPE_CNFG_REG32 0x%x\n",*NFI_NAND_TYPE_CNFG_REG32);
