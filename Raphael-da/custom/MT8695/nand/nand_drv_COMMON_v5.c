...
status_t  NAND_COMMON_BlockErase_V5(
	...
	LOGE("[BE] 0x%x  slc %d\n", page_addr,g_pNandInfo->m_dev->m_tlc_setting->slcopmodeEn);
#if CFG_2CS_NAND
	if(g_bTricky_CS)
	{
...
void dump_nfi(void) {
	...
	//LOGE("%x %x %x %x\n", *((volatile uint32 *)(0x1100D000+cnt*16+0)),*((volatile uint32 *)(0x1100D000+cnt*16+4)),*((volatile uint32 *)(0x1100D000+cnt*16+8)),*((volatile uint32 *)(0x1100D000+cnt*16+12)));  //therm_ctrl status?
}
...
status_t  NAND_COMMON_PageRead_V5_hw(
	...
	NFI_TLC_WL_INFO  tlc_wl_info;
       ...
       //U32   nfi_sector_num = g_pNandInfo->m_dev->m_hw_info.m_page_size /g_pNandInfo->m_dev->m_hw_info.m_sector_size;
#endif
	LOGD("[RP] 0x%x  0x%x 0x%x  slc %d\n", row_addr, (U32)p_data32, (U32)p_spare32, g_pNandInfo->m_dev->m_tlc_setting->slcopmodeEn);
...
#endif
		//	   LOGE("line=%d,function= %s \n ",	__LINE__,  __FUNCTION__);
	LOGD(" qq   ,line=%d,function= %s,row_addr=0x%x, p_data32=0x%x, data 0x%x, fgslc = %d \n ",	__LINE__,  __FUNCTION__, row_addr,p_data32,*p_data32, g_pNandInfo->m_dev->m_tlc_setting->slcopmodeEn);
	LOGD("first 16 bytes: 0x%x 0x%x 0x%x 0x%x\n", p_data32[0],p_data32[1],p_data32[2],p_data32[3]);
	LOGD("last 32 bytes: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
		p_data32[4088],p_data32[4089],p_data32[4090],p_data32[4091],
		p_data32[4092],p_data32[4093],p_data32[4094],p_data32[4095]);
	/*if(row_addr == 0x78600)
	{
		for(i=0;i<256;i++){
			LOGE("0x%x ", p_data32[i]);
			if (((i+1) % 16) == 0)
			LOGD("\n");
		}
	}*/
